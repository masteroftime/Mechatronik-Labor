/*
 * plattform independent function (no include files)!
 */
 ...
 **********************************************
 * This function provides fast approximation to the trigonometric sine function for floating-point data
 */
float Math__sin(  // return  sin(x)
  float x)             // 0..2*PI [radians] angle (for highest precision the angle must be within the Range of 0 and 2*PI)
{
  float sinVal, fract, in;                   /* Temporary variables for input, output */
  unsigned int index;                                /* Index variable */
